// lib/src/pages/tutorials_discharge_information.dart
import 'package:flutter/material.dart';
import 'package:url_launcher/url_launcher.dart';
import '../components/chd_scaffold.dart';

class DischargeInformationPage extends StatelessWidget {
  const DischargeInformationPage({super.key});
    ...
    return ChdScaffold(
      ...
      child: (isArabic) {
        final steps = isArabic
            ? const [
          'راجع قائمة الأدوية وتأكد من الجرعات والأوقات.',
          'حدد موعد المتابعة مع طبيب القلب/الأطفال.',
          'اسأل عن حدود الحركة والاستحمام والعناية بالجرح.',
          'احتفظ بأرقام التواصل للطوارئ.',
        ]
            : const [
          'Review medication list and confirm doses/times.',
          'Schedule follow-up with cardiology/pediatrics.',
          'Ask about activity/bathing limits and wound care.',
          'Keep emergency contact numbers.',
        ];

        // Demo videos
        final pectusVideo = 'https://www.youtube.com/watch?v=dQw4w9WgX';
        final headToCheckVideo = 'https://www.youtube.com/watch?v=dQw4w9WgX';

        return ListView(
          children: [
            _Box(
              title: isArabic ? 'قائمة الخروج' : 'Discharge checklist',
              icon: Icons.checklist_outlined,
              child: _Bullets(bullets: steps, isArabic: isArabic),
            ),
            _Box(
              title: isArabic ? 'فيديو: خروج البيكتس ' : 'Video: Pectus discharge ',
            ...
            _Box(
              title: isArabic ? 'فيديو: فحص الرأس ' : 'Video: Head-to-check ',
              icon: Icons.play_circle_outline,
              child: _LinkButton(
                label: isArabic ? 'فتح الفيديو' : 'Open video',
                url: headToCheckVideo,
                isArabic: isArabic,
                onOpen: (u) => _openUrl(context, isArabic, u),
              ),
            ),
